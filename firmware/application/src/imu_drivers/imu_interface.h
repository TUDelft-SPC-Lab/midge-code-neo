/**
 * @brief This files defines the minimum expected functions from any IMU
...
struct __attribute__((packed)) Imu3AxisSample {
    float x;
    float y;
    float z;
};
...
struct ImuDriverInterface {
     ...
     *
     * @param config Pointer to ImuConfig struct containing the desired configuration.
     * @return int status code, 0 for success, negative for error.
     */
    int (*set_config)(struct ImuConfig* config);
...
extern struct ImuDriverInterface imu_driver_interface;
